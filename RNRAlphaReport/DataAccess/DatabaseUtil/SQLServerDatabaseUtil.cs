...
{
    public class SQLServerDatabaseUtil : IDatabaseUtil
    {

        public SQLServerDatabaseUtil()
        {
        ...
        public int ExecuteNonQuerySingleOrBulk(string connectionString, string query, IList<IDictionary<string, object>> parameters = null)
        {
            ...
            try
            {
                ...
                for (var nonQueryLineNo = 0; nonQueryLineNo < nonQueries.Count(); nonQueryLineNo++)
                {
                    ...
                    {
                        using (SqlCommand cmd = new SqlCommand(trimmedQuery, conn, transaction))
                        {
                            //injecting params
                            ...
                            if (match.Success)
                            {
                                ...
                                cmd.Parameters.AddWithValue(paramPlaceholder, value);
                            }
